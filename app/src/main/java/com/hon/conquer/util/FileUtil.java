...
public class FileUtil {
    ...
    private static final String PICTURE_PATH = Environment.getExternalStorageDirectory() + File.separator
            + BuildConfig.APPLICATION_ID + File.separator + "Conquer";
        ...
        try {
            ...
            if (!dir.exists()) {
                if (dir.mkdirs()) {
                    ...
                    ConquerExecutors.getInstance().getMainExecutor().execute(
...
//                    targetFile.getPath(), targetFile.getName(), null);
        ...
        Conquer.sConquer.sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, uri));
    }
}
